defmodule PremiereEcouteWeb.SessionChannelTest do
  ...
  setup do
    user = user_fixture(%{role: :streamer, twitch_user_id: "1234"})
  ...
  describe "push - vote" do
    test "receive current score when a new vote is casted to the active listening session", %{user: user} do
      EventBus.dispatch(%MessageSent{broadcaster_id: user.twitch_user_id, user_id: "viewer1", message: "5", is_streamer: false})
      EventBus.dispatch(%MessageSent{broadcaster_id: user.twitch_user_id, user_id: "viewer2", message: "10", is_streamer: false})
