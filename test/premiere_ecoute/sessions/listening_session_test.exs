defmodule PremiereEcoute.Sessions.ListeningSessionTest do
  ...
  describe "previous_track/1 - playlist" do
    setup %{user: user, playlist: playlist} do
      track_attrs = fn name, track_id ->
        %{
          provider: :spotify,
    ...
    test "cannot select the previous track when no current track is selected", %{session: session} do
      {:error, reason} = ListeningSession.previous_track(session)

      assert reason == :no_tracks_left
    end

    test "cannot select the previous track when the first track is selected", %{session: session} do
      {:ok, session} = ListeningSession.next_track(session)
      {:error, reason} = ListeningSession.previous_track(session)
    ...
    test "selects the first track when the second track is selected", %{session: session, track1: track1} do
      {:ok, session} = ListeningSession.next_track(session)
      {:ok, session} = ListeningSession.next_track(session)
