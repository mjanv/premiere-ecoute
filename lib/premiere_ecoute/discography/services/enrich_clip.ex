defmodule PremiereEcoute.Discography.Services.EnrichClip do
  ...
  def resolve_single(youtube_video_id) when is_binary(youtube_video_id) do
    with {:ok, video} <- Apis.youtube().get_video(youtube_video_id),
         {:ok, %Single{} = single} <- find_single(video, youtube_video_id) do
      {:ok, Single.put_artist(single), video.thumbnail_url}
    end
  end

  defp find_single(video, youtube_video_id) do
    case Single.get_by_provider_id(:youtube, youtube_video_id) do
      %Single{} = single -> {:ok, single}
      nil -> with {:ok, single} <- find_spotify_match(video), do: attach_youtube_id(single, youtube_video_id)
    end
  end
