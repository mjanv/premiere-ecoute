defmodule PremiereEcouteWeb.Sessions.SessionSelectionLive do
  ...
  def handle_event("fetch_currently_playing", _params, socket) do
    ...
    |> start_async(:select_track, fn ->
      case PremiereEcoute.Apis.cache(:spotify).get_playback_state(scope, PlaybackState.default()) do
        {:ok, %PlaybackState{item: %{uri: "spotify:track:" <> track_id} = item}} when not is_nil(item) ->
          PremiereEcoute.Apis.spotify().get_single(track_id)
  ...
  def handle_event(
        "prepare_session",
        _params,
        %{assigns: %{selected_clip_video: %{result: video}}} = socket
      )
  ...
  defp update_state(socket) do
    ...
    content_selected =
      (source_type == "album" && selected_album.ok? && selected_album.result) ||
        ...
        (source_type == "clip" && selected_clip_video.ok? && selected_clip_video.result) ||
